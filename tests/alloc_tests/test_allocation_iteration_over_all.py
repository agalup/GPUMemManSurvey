...
def main():
    ...
    #            with open(csv_path_free, "a", newline='') as csv_file:
    #                csv_file.write("\n" + str(allocation_size) + ",")
    #            run_config = str(num_allocations) + " " + str(allocation_size) + " " + str(num_iterations) + " " + str(85) + " " + str() + " " + str(85) + " " + str(256) + " " +  str(measure_on_device) + " " + str(test_warp_based) + " 1 " + str(free_memory) + " " + csv_path_alloc + " " + csv_path_free + " " + str(args.allocsize) + " " + str(args.device)
    ...
    if run_testcases:
        # Run Testcase
        for name, path in testcases.items():
            csv_path_alloc = "results/performance/" + warpstring + "perf_iteration_over_app-sm-108-52_mm-sm-2-50_alloc_" + name + "_" + str(num_allocations) + "_" + str(largest_allocation_size) + "_" + str(num_iterations) + ".csv"
            csv_path_free = "results/performance/" + warpstring + "perf_iteration_over_all_free_" + name + "_" + str(num_allocations) + "_" + str(largest_allocation_size) + "_" + str(num_iterations) + ".csv"
            if(os.path.isfile(csv_path_alloc)):
                print("This file <" + csv_path_alloc + "> already exists, do you really want to OVERWRITE?")
                inputfromconsole = input()
                if not (inputfromconsole == "yes" or inputfromconsole == "y"):
            ...
            with open(csv_path_alloc, "w", newline='') as csv_file:
            #with open(csv_path_alloc, "a", newline='') as csv_file:
                csv_file.write("AllocationSize (in Byte), iter, max_sm, mean, std-dev, min, max, median, mm_sm, app_sm, mm_bl, mm_th, app_bl, app_th, app warps per mm warp")
            with open(csv_path_free, "w", newline='') as csv_file:
            #with open(csv_path_free, "a", newline='') as csv_file:
                csv_file.write("AllocationSize (in Byte), iter, max_sm, mean, std-dev, min, max, median, mm_sm, app_sm, mm_bl, mm_th, app_bl, app_th, app warps per mm warp, active warps mm, active warps app")
#            allocation_size = smallest_allocation_size
            ...
            #iter_mm_bl = 1
            #while allocation_size <= largest_allocation_size:
            #while iter_mm_sm <= 108:
            iteration = 1
            for max_sm in range(108, 50, -2):
                for iter_app_sm in range(max_sm-2, 48, -2):
                    iter_mm_sm = max_sm - iter_app_sm
                    iter_mm_bl = iter_mm_sm
                    mm_th = 1024
                    app_th = 1024
                    #while iter_app_sm <= 108:
                    #while iter_mm_bl <= iter_mm_sm:
                    iteration = iteration + 1
                    with open(csv_path_alloc, "a", newline='') as csv_file:
                        csv_file.write("\n" + str(allocation_size) + "," + str(iteration) + "," + str(max_sm) + ",")
                    with open(csv_path_free, "a", newline='') as csv_file:
                        csv_file.write("\n" + str(allocation_size) + "," + str(iteration) + "," + str(max_sm) + ",")
                    run_config = str(num_allocations) + " " + str(allocation_size) + " " + str(num_iterations) + " " + str(iter_mm_sm) + " " + str(iter_app_sm) + " " + str(iter_mm_bl) + " " + str(mm_th) + " " +  str(app_th) + " " + str(measure_on_device) + " " + str(test_warp_based) + " 1 " + str(free_memory) + " " + csv_path_alloc + " " + csv_path_free + " " + str(args.allocsize) + " " + str(args.device)
                    app_bl = (num_allocations + 1023)/1024
                    app_th_all = app_bl * 1024
                    mm_th_all = iter_mm_bl * mm_th
                    warp_throughput = (app_th_all + mm_th_all - 1)/mm_th_all
                    active_warps_mm = (mm_th / 32) * iter_mm_sm
                    active_warps_app = (app_th * 2 / 32) * iter_app_sm
                    executecommand = "{0} {1}".format(path, run_config)
                    print("#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#")
                    print("Running " + name + " with command -> " + executecommand)
                    print("#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#")
                    ...
                    if process_killed :
                        ...
                        print("Success!")
                        with open(csv_path_alloc, "a") as csv_file:
                            param = "," + str(iter_mm_sm) + "," + str(iter_app_sm) + "," + str(iter_mm_bl) + "," + str(mm_th) + "," + str(app_bl) + "," + str(app_th) + "," + str(warp_throughput) + "," + str(active_warps_mm) + "," + str(active_warps_app)
                            print("adding params: ", param)
                            csv_file.write(param)
              #              iter_mm_bl = iter_mm_bl + 1
              #          iter_app_sm = iter_app_sm + 2
              #      iter_mm_sm = iter_mm_sm + 2
            if args.mailpass:
                message = "Testcase {0} ran through! Testset: ({1})".format(str(name), " | ".join(testcases.keys()))
                #print("no mail to be send");
                #mailalert.sendAlert(message)
